...
## Present the data
        ...
        log_plot = log_ret.hvplot.hist(bins=100, subplots=True, width=400, group_label='Ticker', grid=True).cols(2)
        ...
        # st.pyplot(sns.heatmap(correlation, vmin=-1,vmax=1)) - error with mathplotlib
    ...
    # Best weights
            ...
            scatter_plot = hv.Scatter((vol_arr, ret_arr, sharpe_arr), 'Volatility', ['Return', 'Sharpe Ratio'])
            max_sharpe_plot = hv.Scatter([(max_sr_vol,max_sr_ret)])
        ...
        # MC simulation- problem with index levels from df
        ...
        #st.plotly_chart(dist_plot) # not working with mathplotlib
        ...
        # for ind in range(num_ports): - this is out of order
#         #st.write(hv.render(scatter_plot, backend='bokeh')) does not plot-problem with hvplot
